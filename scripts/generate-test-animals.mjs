...
const plan = JSON.parse(
  readFileSync(new URL("./meshy-test-animals.json", import.meta.url), "utf8"),
);
...
const common =
  "One full-body wildlife game asset. Natural proportions, gently stylized realistic appearance matching a textured zebra. Neutral standing pose, feet on one plane, limbs clearly separated, facing forward. Animal only: no ground, scenery, pedestal, text, collar or accessories.";
...
if (planned.length !== 9 || planned.some((a) => a.prompt.length > 800))
  throw new Error("Invalid bounded batch plan");
if (dryRun) {
  console.log(
    JSON.stringify(
      {
        batch: plan.batch,
        animals: planned.map((a) => ({ id: a.id, prompt: a.prompt })),
        estimatedCredits: 9 * plan.estimatedCreditsPerAnimal,
      },
      null,
      2,
    ),
...
if (existsSync(stateFile) && resumeIndex < 0)
  throw new Error(
    "Existing state found; explicitly resume it to avoid duplicate charges",
  );
const state =
  resumeIndex >= 0
    ...
    : {
        ...
        previewModel: plan.previewModel,
        textureModel: plan.textureModel,
        animals: planned,
      };
if (
  state.batch !== plan.batch ||
  state.animals.length !== 9 ||
  state.animals.some((a, i) => a.id !== planned[i].id)
)
  throw new Error("Resume state does not match batch");
function save() {
  writeFileSync(`${stateFile}.tmp`, JSON.stringify(state, null, 2) + "\n");
  // Windows scanners can briefly lock the destination. Retry only this atomic
  // checkpoint replacement; never unlink the old state or repeat a paid request.
...
function safeError(error) {
  ...
  return String(error?.message || "Unexpected generation failure")
    .split(key)
    .join("[redacted]")
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Meshy ${method} returned HTTP ${response.status}; ${body ? "submission was not retried" : "request stopped"}`,
        );
      }
...
async function submit(animal, stage, body) {
  ...
  if (animal[stage]?.submissionStartedAt)
    throw new Error(
      `${animal.id} ${stage}: uncertain prior submission; reconcile Meshy task list before resuming`,
    );
  animal[stage] = {
  ...
  if (typeof task.result !== "string" || !task.result)
    throw new Error(
      "Meshy did not return a task ID; reconcile before retrying",
    );
...
async function awaitTask(animal, stage, id) {
  ...
  while (Date.now() < deadline) {
    ...
    Object.assign(animal[stage], {
      status: result.status,
      progress: result.progress,
      consumedCredits: result.consumed_credits ?? null,
    });
    save();
    const progress = `${result.status} ${result.progress ?? 0}%`;
    if (progress !== lastProgress) {
      console.log(`${animal.id}: ${stage} ${progress}`);
      lastProgress = progress;
    }
    if (result.status === "SUCCEEDED") return result;
    if (["FAILED", "CANCELED"].includes(result.status))
      throw new Error(
        `${animal.id} ${stage} ${result.status}; task retained for inspection`,
      );
    await delay(20000);
  }
  throw new Error(
    `${animal.id} ${stage} polling timed out; resume the saved task ID`,
  );
}
async function download(urlString, filename, kind) {
  ...
  if (
    ...
    throw new Error("Unexpected Meshy asset host; inspect before downloading");
  ...
  if (buffer.length > 80 * 1024 * 1024)
    throw new Error("Asset exceeds the test batch size limit");
  if (
    kind === "glb" &&
    (buffer.length < 28 ||
...
async function generate(animal) {
  ...
  const refineId = await submit(animal, "refine", {
    ...
    ai_model: plan.textureModel,
    texture_prompt: animal.texturePrompt,
...
try {
  ...
  if (!Number.isFinite(balance))
    throw new Error("Meshy balance could not be verified");
  const estimatedRemaining = state.animals.reduce(
    (sum, a) => sum + (!a.preview?.id ? 5 : 0) + (!a.refine?.id ? 10 : 0),
    0,
  );
  ...
  save();
  console.log(
    `Authenticated. Estimated remaining batch cost: ${estimatedRemaining} credits. Available API credits: ${balance}.`,
  );
  if (balance < estimatedRemaining)
    throw new Error(
      `Insufficient API credits for this batch: need ${estimatedRemaining}, available ${balance}`,
    );
  ...
  await Promise.all(
    Array.from({ length: 3 }, async () => {
      while (next < state.animals.length) {
        ...
        try {
          await generate(animal);
        } catch (error) {
          animal.status = "failed";
          animal.error = safeError(error);
          save();
          console.error(`${animal.id}: ${animal.error}`);
        }
      }
    }),
  );
  state.finishedAt = new Date().toISOString();
  state.consumedCredits = state.animals.reduce(
    (sum, a) =>
      sum +
      (a.preview?.consumedCredits || 0) +
      (a.refine?.consumedCredits || 0),
    0,
  );
  save();
    ...
    JSON.stringify(
      {
        batch: state.batch,
        reviewStatus:
          "AI-generated test assets; anatomy review and animation pending",
        animals: ready.map(
          ({
            id,
            name,
  ...
  console.log(
    `Batch complete: ${ready.length}/9 textured test animals. Reported consumption: ${state.consumedCredits} credits.`,
  );

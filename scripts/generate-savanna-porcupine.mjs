/** One reviewed replacement image-to-3D tasks, never an automatic paid retry.
 ...
 * https://docs.meshy.ai/en/api/pricing — 1 textured standard task × 30 credits.
 */
import {
  readFileSync,
  writeFileSync,
...
const output = resolve(
  process.env.MESHY_OUTPUT_DIR || join(project, "generated/savanna-porcupine"),
);
const stateFile = join(output, "batch-state.json");
const key = process.env.MESHY_API_KEY;
const ids = ["cape-porcupine"];
...
async function main() {
  const plan = JSON.parse(
    readFileSync(new URL("./savanna-porcupine.json", import.meta.url), "utf8"),
  );
  ...
  if (
    plan.batch !== "savanna-porcupine-20260921-v1" ||
    plan.concurrency !== 2 ||
    ...
    plan.assets.some(
      (asset, i) =>
        asset.id !== ids[i] ||
        asset.reference !==
          `assets/references/savanna-porcupine/${asset.id}.png` ||
        asset.targetPolycount !== 15_000,
    )
  )
    throw new Error(
      "Plan differs from the authorized one-porcupine revision batch",
    );
  ...
  if (process.argv.includes("--dry-run")) {
    console.log(
      JSON.stringify(
        { batch: plan.batch, estimatedCredits: 30, assets: planned },
        null,
        2,
      ),
    );
  ...
  console.log(
    `Starting bounded batch: at most 1 task, ${estimatedRemaining} estimated remaining credits.`,
  );
  let next = 0;
  ...
  console.log(
    `Finished: ${ready}/1 models with local review thumbnails. Reported consumption: ${state.consumedCredits} credits.`,
  );
  if (ready !== 1) process.exitCode = 1;
}

/** Three reviewed replacement image-to-3D tasks, never an automatic paid retry.
 ...
 * https://docs.meshy.ai/en/api/pricing — 3 textured standard tasks × 30 credits.
 */
import {
  readFileSync,
  writeFileSync,
...
const output = resolve(
  process.env.MESHY_OUTPUT_DIR || join(project, "generated/savanna-revisions"),
);
const stateFile = join(output, "batch-state.json");
const key = process.env.MESHY_API_KEY;
const ids = ["cape-porcupine", "south-african-springhare", "bat-eared-fox"];
...
async function main() {
  const plan = JSON.parse(
    readFileSync(new URL("./savanna-revisions.json", import.meta.url), "utf8"),
  );
  ...
  if (
    plan.batch !== "savanna-revisions-20260921-v1" ||
    plan.concurrency !== 2 ||
    ...
    plan.assets.some(
      (asset, i) =>
        asset.id !== ids[i] ||
        asset.reference !==
          `assets/references/savanna-revisions/${asset.id}.png` ||
        asset.targetPolycount !== 15_000,
    )
  )
    throw new Error("Plan differs from the authorized 3-asset revision batch");
  ...
  if (process.argv.includes("--dry-run")) {
    console.log(
      JSON.stringify(
        { batch: plan.batch, estimatedCredits: 90, assets: planned },
        null,
        2,
      ),
    );
  ...
  console.log(
    `Starting bounded batch: at most 3 tasks, ${estimatedRemaining} estimated remaining credits.`,
  );
  let next = 0;
  ...
  console.log(
    `Finished: ${ready}/3 models with local review thumbnails. Reported consumption: ${state.consumedCredits} credits.`,
  );

/** Four explicitly requested image-to-3D tasks, never an automatic paid retry.
 ...
 * https://docs.meshy.ai/en/api/pricing — 4 textured standard tasks × 30 credits.
 */
import {
  readFileSync,
  writeFileSync,
...
const output = resolve(
  process.env.MESHY_OUTPUT_DIR || join(project, "generated/safari-assets"),
);
const stateFile = join(output, "batch-state.json");
const key = process.env.MESHY_API_KEY;
const ids = ["lion", "common-ostrich", "hippopotamus", "safari-jeep"];
...
async function main() {
  const plan = JSON.parse(
    readFileSync(
      new URL("./safari-image-assets.json", import.meta.url),
      "utf8",
    ),
  );
  ...
  if (
    plan.batch !== "safari-image-assets-20260920-v1" ||
    plan.concurrency !== 2 ||
    plan.estimatedCreditsPerAsset !== 30 ||
    JSON.stringify(plan.request) !== JSON.stringify(expectedRequest) ||
    plan.assets?.length !== 4 ||
    plan.assets.some(
      (asset, i) =>
        asset.id !== ids[i] ||
        asset.reference !== `assets/references/${asset.id}.png` ||
        asset.targetPolycount !==
          (asset.id === "safari-jeep" ? 25_000 : 20_000),
    )
  )
    throw new Error("Plan differs from the authorized four-asset batch");
  ...
  if (process.argv.includes("--dry-run")) {
    console.log(
      JSON.stringify(
        { batch: plan.batch, estimatedCredits: 120, assets: planned },
        null,
        2,
      ),
    );
  ...
  console.log(
    `Starting bounded batch: at most four tasks, ${estimatedRemaining} estimated remaining credits.`,
  );
  let next = 0;
  ...
  console.log(
    `Finished: ${ready}/4 models with local review thumbnails. Reported consumption: ${state.consumedCredits} credits.`,
  );
  if (ready !== 4) process.exitCode = 1;
}
